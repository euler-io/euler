package com.github.euler.common;

public interface CommonMetadata {

    final static String CREATED_DATETIME = "created-datetime";
    final static String LAST_MODIFIED_DATETIME = "last-modified-datetime";
    final static String NAME = "name";
    final static String SIZE = "size";
    final static String IS_DIRECTORY = "is-directory";
    final static String PATH = "path";
    final static String RELATIVE_PATH = "relative-path";
    final static String FULL_PATH = "full-path";
    final static String MIME_TYPE = "mime-type";
    final static String ITEM_URI = "item-uri";

}
